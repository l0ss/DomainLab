...
if (!([ADSI]::Exists("LDAP://OU=Servers,DC=labco,DC=local")))
{
    New-ADOrganizationalUnit -Name "Servers" -Server "dc.labco.local"
}
else
{
...
if (!([ADSI]::Exists("LDAP://OU=Workstations,DC=labco,DC=local")))
{
    New-ADOrganizationalUnit -Name "Workstations" -Server "dc.labco.local"
}
else
{
