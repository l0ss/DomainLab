...
.LINK
https://blogs.technet.microsoft.com/undocumentedfeatures/2018/05/29/update-to-create-labusers-tool/

.NOTES
2018-06-28
- Added resource mailbox provisioning.
2018-06-27
- Updated $MaxRecipients calculation to avoid error when using -Count parameter with 1 user
- Added "MiddleName" attribute
2018-06-18
- Updated default Password value to a more complex password to avoid substring matches if the complex password policy is enabled
- Updated sAMAccountName and userPrincipalName values to remove space characters, since those caused provisioning errors
2018-06-13	
- Updated CreateOU function logic. Moved previous logic to deprecated function.
- Updated parameters to be more independent, so you can specify just creating groups, just inflating mailboxes, etc.
- Added -DomainController parameter to help with consistency in large lab environments.
2018-06-12
- Added random binary attachments to InflateMailboxes parameter
- Removed Count as a mandatory parameter; will allow just sending mail or creating group membership assignments
2018-06-05
...
- Fixed manager assignment issue.  In some cases, managers were	being assigned to users outside of the correct scope.
2018-04-25
- Initial release.
#>
	...
	If ($CreateResourceMailboxes -and $ExchangeServer)
	{
		...
		foreach ($Location in $Locations.State)
		{
			...
			If ($Rooms)
			{
				Try
				{
					$RoomList = Get-DistributionGroup -Identity "All Rooms - $($Location)" -DomainController $DomainController -ea silentlycontinue
				}
				Catch
				{ }
				...
				If ($RoomList)
				{
					Write-Log -Logfile $Logfile -LogLevel INFO -Message "Updating room list 'All Rooms - $($Location)'" -ConsoleOutput
					$Members = Get-DistributionGroupMember -Identity $RoomList.Alias -DomainController $DomainController
					$Members | % { Remove-DistributionGroupMember -confirm:$false -Identity $RoomList.Alias -Member $_.Alias -DomainController $DomainController -ea silentlycontinue }
					$Rooms | % { Add-DistributionGroupMember -Identity $RoomList.Alias -Member $_.Alias -ea silentlycontinue }
				}
				Else
				{
					Write-Log -Logfile $Logfile -LogLevel INFO -Message "Creating room list 'All Rooms - $($Location)'" -ConsoleOutput
					$NewRoomList = New-DistributionGroup -Name "All Rooms - $($Location)" -OrganizationalUnit $GroupsOU -DomainController $DomainController
		...
		foreach ($Location in $Locations.State)
		{
			for ($i = 0; $i -lt $EquipmentMailboxes.Name.Count; $i++)
			{
				...
				Try { $GLOBAL:UserExists = Get-ADUser -Server $DomainController -LDAPFilter "(sAMAccountName=$sAMAccountName)" }
				Catch { }
				If ($UserExists)
				{
					$i--
				...
				try
				{
					$global:User = New-Mailbox -SamAccountName $sAMAccountName -Name $DisplayName -OrganizationalUnit $OrgPath `
										-DisplayName $DisplayName -PrimarySmtpAddress "$sAMAccountName@$Domain" `
...
function InflateMailboxes
{
	...
	ConnectToExchange
	[array]$Users = Get-Mailbox -DomainController $DomainController -Resultsize Unlimited -OrganizationalUnit $OUPath
	[int]$UsersCount = $Users.Count
	[int]$UserCounter = 1
...
If ($Count)
{
	# If Count is specified AND CreateMailboxes is specified, create mailbox users
	If ($CreateMailboxes) { CreateMailboxes }
	
	# If count is specified but CreateMailbox is NOT specified, create AD users
	Else { CreateADUsers }
}
If ($CreateGroups)
{
...
If ($CreateResourceMailboxes) { CreateResourceMailboxes }
